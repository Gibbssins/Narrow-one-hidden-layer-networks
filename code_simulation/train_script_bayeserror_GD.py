...
# my own
import utils as ut
from utils import check_string, extract_alpha, safe_save, load_files_as_list, compare_dicts, append_unique_line, rank_column_highest_on_diagonal, get_all_parameters, sort_indices 
...
parser.add_argument("--save_dir", default="results", help="save dir")
...
if not initialize_previousepochs:
    ...
    if teacher_student:
        ...
        err_test = ((output_test - y_test)**2).sum().item() / num_test
        print(f"The test error without learning is {err_test}")
    else:
        _, output_train = net(X)
        err_train = ((output_train - y)**2).sum().item() / num_train
        print(f"The train error without learning is {err_train}")
    
    results = {}
    losses, errs, normWs, Qst00s, Qst01s, Qss00s, Qss01s, std_errs_test, errs_test = [], [], [], [], [], [], [], [], []
    ...
    if teacher_student: 
        try:
            std_errs_test = results['std_errs_test']
...
# print (f"the student output prediciton is {output.detach()} after substracting the mean")
# Initialize NVML to access GPU info
pynvml.nvmlInit()
handle = pynvml.nvmlDeviceGetHandleByIndex(0)

# Shared variable to hold utilization samples
util_samples = []
monitoring = [False]

# Function to monitor GPU utilization in the background
def monitor_gpu_utilization(interval=0.01):
    while monitoring[0]:
...
for ep in range(1, num_epochs + 1):
    ...
    # Start monitoring
    monitoring[0] = True
    t = threading.Thread(target=monitor_gpu_utilization)
    t.start()
    ...
    if teacher_student:
        ...
        # Suppose output_test and y_test are your predicted and true values (both tensors)
        squared_errors = (output_test - y_test) ** 2

        # Mean squared error (as you already have)
        err_test = squared_errors.sum().item() / num_test

        # Standard deviation of squared errors
        std_err_test = squared_errors.std(unbiased=True).item()
        std_errs_test.append(std_err_test)
    ...
    if ep % print_every == 0:
        ...
        try:
            if p_train==number_best_model:
                ...
                bayes_err_test = ((bayesian_prediction - y_test)**2).sum().item() / num_test 
                bayes_errs_test.append(bayes_err_test)
                del output_testgf
                p_train=0
                print(f"the bayesian predicition is {bayesian_prediction[:3].detach()} and the true predicition is {y_test[:3].detach()} and the last prediction was {output_testg[:3].detach()}")
            else:
                #check if improved train
    ...
    if teacher_student: 
        ...
        result['std_errs_test'] = std_errs_test
        try:
    ...
    # Get GPU memory and average utilization
    mem_info = pynvml.nvmlDeviceGetMemoryInfo(handle)
    avg_util = sum(util_samples) / len(util_samples) if util_samples else 0
    util_samples.clear()

    print(f"Epoch {ep} took {end_time - start_time:.2f}s")
    ...
    print(f"Epoch {ep+1} took {epoch_duration:.2f}s")
    print(f"GPU memory used: {mem_info.used / 1024**2:.2f} MB")
